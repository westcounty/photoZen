...
#### 📱 底部导航栏系统（新增）
- **4 Tab 导航**：首页 / 时间线 / 相册 / 设置，一键直达
- **状态保持**：Tab 切换时页面状态自动保持
- **全屏沉浸**：整理流程页面自动隐藏底部导航，专注体验

#### 👆 手势交互统一
- **统一手势规范**：所有照片列表页面手势行为一致
  - 点击 = 查看大图/进入详情
  - 长按 = 进入选择模式
  - 长按 + 拖动 = 批量选择
- **应用范围**：相册照片列表、时间线、回收站、保留照片列表

#### 🏠 首页信息架构重构
- **分层卡片设计**：主操作区 + 快捷入口 + 统计信息
- **统一整理入口**：合并"快速整理"和"一站式整理"为一个入口
- **整理模式选择**：点击"开始整理"弹出模式选择框
...
#### 💡 交互引导系统（新增）
- **滑动整理引导**：首次进入显示左滑/右滑/上滑操作引导
- **长按多选引导**：首次进入照片列表显示长按提示
- **引导持久化**：已看过的引导不再重复显示
- **设置重置**：可在设置中重置所有引导

#### ⚡ 批量操作效率优化
- **选择顶栏**：显示"已选择 X 张"，提供全选/取消按钮
- **快捷退出**：点击关闭按钮或返回键退出选择模式
- **撤销支持**：批量操作后显示 Snackbar，支持撤销

#### 🔍 筛选条件管理
- **Chip 行显示**：顶部显示当前生效的筛选条件
- **快速清除**：点击 Chip 上的 X 清除单个条件
- **筛选面板**：底部抽屉形式，支持相册和日期筛选
- **预设保存**：最多保存 3 个筛选预设
...
#### 📊 整理统计页面（新增）
- **日历热力图**：类似 GitHub 贡献图的设计，展示近 12 周整理活动
...
- **性能优化**：使用 `drawBehind` 和 `graphicsLayer` 高效绘制

#### 🔲 空状态统一
- **统一设计语言**：所有空状态使用一致的视觉风格
- **场景化图标**：回收站空、相册空、时间线空等各有专属图标
- **引导文案**：空状态配有操作引导

#### ⚙️ 设置页面重构
- **分组折叠**：设置项按功能分组，可展开/折叠
- **视觉层次**：更清晰的分组标题和分隔
- **交互优化**：开关、滑块等控件操作更便捷

#### 📳 震动反馈系统
- **临界点反馈**：滑动达到触发阈值时轻微震动提示
- **操作完成反馈**：不同操作有不同震动模式
- **可配置开关**：设置中可开启/关闭震动反馈

#### 💬 全局 Snackbar 系统
- **统一管理**：所有页面的 Snackbar 由全局管理器统一处理
- **样式区分**：成功（绿色）、错误（红色）、警告（黄色）
- **撤销支持**：带操作按钮的 Snackbar 支持撤销功能

#### 🗑️ 删除确认优化
- **预览弹窗**：批量删除前显示待删除照片的缩略图预览（最多 9 张）
- **操作区分**：移入回收站 vs 永久删除采用不同警告级别
- **安全保障**：取消按钮明显，避免误操作
...
- **导航架构**：新增 `MainDestination`、`MainScaffold`、`MainBottomNavigation`
- **引导系统**：新增 `GuideRepository`、`GuideTooltip`、`GuideState`
- **筛选系统**：新增 `FilterConfig`、`FilterPresetRepository`、`FilterBottomSheet`
- **批量操作**：新增 `SelectionTopBar`、`UndoManager`
- **统计数据**：新增 `SortingRecordEntity`、`StatsRepository`
- **视觉组件**：新增 `EdgeGlow`、`DirectionIndicator`、`EmptyState`、`ConfirmDeleteSheet`
- **状态管理**：新增 `SnackbarManager` 全局状态管理
- **性能优化**：热力图缓存计算，动画使用 `graphicsLayer`
