...
#### 地理位置解析重构
- **新增 GeoLocationResolver**：统一地理位置解析入口，首次从 EXIF 读取 GPS 后自动缓存到数据库
- 后续所有场景（重命名、小部件、全屏预览、卡片信息）直接命中数据库，跳过重复 EXIF IO
- 正式启用 `gpsScanned` 标记，已扫描无 GPS 的照片不再反复尝试读取
...
#### 相册"从此开始"
- 卡片视图正确定位到选中照片开始整理
- 退出后自动恢复之前的待整理照片范围设置，不再覆盖为"自定义"

#### 重命名功能
- 自增序号改为"起始值 + 步进值"模式，去掉固定位数/自然数字选项
- 元素标签显示为"序号：从N开始"
- 文件名预览框实时显示地理位置省市信息
...
- 修复相册照片列表"从此开始"按钮无反应的问题
- 修复重命名时地理位置省市获取不到的问题（EXIF fallback）
- 修复重命名预览中地理位置始终为空的问题
