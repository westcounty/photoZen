...
### 🗂️ 照片管理
- 按状态查看照片（保留/待定/回收站）
- **列数切换** - 所有瀑布流界面支持 1/2/3 列切换，偏好自动保存
- **批量管理** - 多选后批量操作（保留/待定/删除/设置标签/重置）
- 长按照片快速切换状态或编辑
- 回收站支持多选和系统级彻底删除
...
- **丝滑拖动** - 气泡跟手移动、惯性滑动、弹性碰撞
- **位置记忆** - 自动保存气泡位置，下次进入时恢复布局
...
### V1.5.7 (2026-01-16)
- 🔧 **Flow筛选修复** - 筛选条件（如排除相机照片）现在正确生效
- 📸 **会话照片追踪** - 待定筛选和快速打标只显示本次Flow中的照片
- 📊 **列数切换** - 所有瀑布流界面支持1/2/3列切换，设置自动保存
- ❓ **回收站增强** - 批量管理新增"设置为待定"操作
- 🏷️ **标签功能增强** - 修改标签时始终显示新建标签选项
- 📐 **回收站优化** - 改为完整展示的双列瀑布流布局
- 🌟 **批量管理扩展** - 待定、回收站、标签照片列表均支持批量操作
